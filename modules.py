...
class Block(nn.Module):
    def __init__(self, h, d, d_ff):
        ...
        self.CausalSelfAttn = CausalSelfAttention(
            d, d_qk=d//h, d_v=d//h, h=h, p=0.2)
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, X: Tensor):          # assume input is Z x B x N x d or N x d
        ...
        AV: Tensor = scaled_dot_product_attention(
            query=Q, key=K, value=V, is_causal=True)  # dim B x h x N x d_v
        AV = torch.transpose(AV, dim0=1, dim1=2)    # B x N x h x d_v
        # B x N x h*d_v, head concat
        AV = torch.reshape(AV, shape=(-1, X.size(-2), self.h * self.d_v))
        SA_out = self.W_O(AV)
        return SA_out
...
def scaled_dot_product_attention(query, key, value, attn_mask=None, dropout_p=0.0, is_causal=True, scale=None) -> torch.Tensor:
    ...
    if is_causal:
        ...
        temp_mask = torch.ones(L, S, dtype=torch.bool).tril(
            diagonal=0).to(query.device)
